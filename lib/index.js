...
export class SpaceTrack {
  ...
  get (queryRaw = {}, optionsRaw = {}) {
    ...
    return result.then( data => {
      if (query.metadata && !Array.isArray(data)) {
        return assoc('data', data, normalise(coerceArray(data.data), options))
      } else {
        return normalise(data, options)
      }
    })
  }
...
const querySchema = Joi.object().keys({
  ...
  metadata:     Joi.boolean().default(false),
