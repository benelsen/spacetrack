import {compose, assoc, dissoc, slice, omit, props, reject, isNil, curry} from 'ramda'
 ...
 * @return {string}
 */
const normaliseDate = (d) => d ? new Date(d).toISOString() : null
const normaliseDateTime = (d) => d ? new Date(`${d}Z`).toISOString() : null
 ...
 * @return {DataEntry[]}
 */
export const normalise = curry((data, options) => data.map(x => normaliseData(x, options)) )
